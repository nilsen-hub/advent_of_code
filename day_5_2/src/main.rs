...
use std::{fs::read_to_string, i64, iter::Enumerate, slice::Chunks};
...
fn map_lookup(from: i64, map: &Vec<(i64, i64, i64)>) -> i64 {
    let mut to = from;
    for map_part in map {
        if from < map_part.1 || from > map_part.1 + map_part.2 - 1 {
            continue;
        } else {
...
fn find_lowest_location(data: &Almanac) -> i64 {
    ...
    let seed_ranges = seed_list_interpreter(data.seeds.clone());
    println!("{:?}", seed_ranges);
    let mut total_count: usize = 0;
    for range in seed_ranges {
        let end = range.1 + range.0;
        let mut here = range.0;
        loop {
            total_count += 1;
            let mut to = here;
            for map in maps {
            ...
            if to < lowest_location {
                ...
                let mut per_sec: usize = 0;
                if elapsed > 0 {
                    per_sec = total_count / elapsed as usize;
                }
                println!(
                    "lowest location updated to {}, it took {} seconds to get here, {} items have been checked, i average {} checks per second",
                    to,
                    elapsed,
                    total_count,
                    per_sec
    ...
    println!(
        "{} is the lowest location, this program took {} seconds to run, {} entries checked",
        lowest_location,
        now.elapsed().as_secs(),
        total_count
    );
...
fn seed_list_interpreter(data: Vec<i64>) -> Vec<(i64, i64)> {
    let mut output: Vec<(i64, i64)> = Vec::with_capacity(50);
...
mod tests {
    ...
    #[test]
    fn interprets_seed_lists() {
        let seeds: Vec<i64> = vec![79, 14, 55, 13];
        let result = seed_list_interpreter(seeds);
