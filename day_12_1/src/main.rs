...
fn condition_map_analysis(map: &ConditionMap){
    ...
    for group in groups{
        ...
        if group.start_index + group.size < bounds{
            if working_map[group.start_index + group.size] == '?' && working_map[group.start_index + group.size + 1].is_numeric() == false{
                step_counter += 1;
                break;
            }
        }
...
fn parse_maps(map_springs: &Vec<char>, map_groups: &Vec<usize>) -> (Vec<char>, Vec<SpringGroup>) {
    ...
    for (index, group_size) in map_groups.iter().enumerate(){
        ...
        'group_loop: loop{
            ...
            if counter + active_window.len() < bounds && working_vector[counter + active_window.len()] == '#' {
                valid_flag = false;
            }
            ...
            if counter == 0{
               ...
               if working_vector[counter - 1].is_numeric() || working_vector[counter + 1].is_numeric(){
                valid_flag = false;
               }
            }
...
fn to_refactor(){
        ...
        for (index, group_size) in groups.iter().enumerate(){
            ...
            'group_loop: loop{
                ...
                if valid_flag && counter == 0 && working_vector[counter + active_window.len()].is_numeric() == false{
                ...
                if valid_flag && counter > 0 && working_vector[counter - 1].is_numeric() == false && working_vector[counter + 1].is_numeric() == false{
                    let mut rollout: usize = group_size.clone();
                    loop{
                        if rollout == 0{
